...
function LifeCycle1() {
    ...
    useEffect(() => {
        let actuelAge = age;
        console.log(`Age actuel : ${actuelAge}`);

        actuelAge >= 18 ? console.log('La personne est majeure') : console.log('La personne est mineure');
        return () => {
            let ancienAge = age;
            console.log(`Ancien age : ${ancienAge}`);
        };
    }, [age]);

    useEffect(() => {
        let temp = degrees;
        temp > 0 ? console.log('Trop chaud !') : console.log('Trop froid !');
        return () => {
